...
for i in tqdm(range(len(annotations_test))):
    ...
    hypotheses.append(output_text.split())

    m.update(([output_text.split()], [annotations_test[i].split()]))
...
annotation_cos_sim /= min(len(annotations_test), 70000)
lyrics_cos_sim /= min(len(annotations_test), 70000)
